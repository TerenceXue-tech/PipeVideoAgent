...
INPUT_SUBDIR="key_frames"
OUTPUT_SUBDIR="key_frames_refined"
SIMILARITY_THRESHOLD="0.98"

RUN_TAG="$(date +%Y%m%d_%H%M%S)"
TMP_DIR="${ROOT_DIR}/tmp_refine_split_${RUN_TAG}"
LOG_DIR="${ROOT_DIR}/logs_refine_split_${RUN_TAG}"
KEEP_TMP_JSON=0
KEEP_LOG_DIR=1
...
echo "[INFO] INPUT_SUBDIR=${INPUT_SUBDIR}"
...
for ((i=0; i<NUM_PROCS; i++)); do
  ...
  SHARD_SUMMARY="${LOG_DIR}/refine_scene_summary_part_$(printf "%02d" "${i}").json"
